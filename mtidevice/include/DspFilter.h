...
public:
  ...
  virtual void setParams( unsigned int numChannels, unsigned int order, double* b, double *a ) {};
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// IIRGeneric.h
#define DSPFILTERS_IIRGENERIC_MAXORDER 32

namespace Dsp {

class IIRGeneric : public Filter
{
public:
	~IIRGeneric() {};

	Kind getKind () const { return kindLowPass; };
	const std::string getName () const { return "IIRGeneric"; }
	int getNumParams () const { return m_iNumTaps; }
	ParamInfo getParamInfo (int index) const { ParamInfo pi; return pi; }
	std::vector<PoleZeroPair> getPoleZeros() const { std::vector<PoleZeroPair> pz; return pz; };
	complex_t response (double normalizedFrequency) const { complex_t ct; return ct; }
	int getNumChannels() { return m_iNumChannels; }
	void reset () {};

	void setParams(  unsigned int numChannels, unsigned int order, double* b, double *a );
	void process (int numSamples, float* const* arrayOfChannels);
	void process (int numSamples, double* const* arrayOfChannels);
...
protected:
	...
	unsigned int m_iNumTaps, m_iNumChannels;
	double m_fCoeffB[ DSPFILTERS_IIRGENERIC_MAXORDER ], m_fCoeffA[ DSPFILTERS_IIRGENERIC_MAXORDER ];
};
